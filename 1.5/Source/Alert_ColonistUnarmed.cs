...
namespace UnarmedAlert
{
    public class Alert_ColonistUnarmed : Alert
    {
        private List<Pawn> GetCulprits()
        {
            ...
            {
                if (pawn.equipment.Primary == null && (pawn.Spawned || pawn.IsCaravanMember()) && (UnarmedAlertSettings.IncludeGuests || (!pawn.HasExtraMiniFaction() && !pawn.HasExtraHomeFaction())) && !pawn.Downed && !pawn.WorkTagIsDisabled(WorkTags.Violent) && (UnarmedAlertSettings.IncludeSlaves || !pawn.IsSlave) && (UnarmedAlertSettings.IncludeChildren || pawn.DevelopmentalStage == DevelopmentalStage.Adult)) culprits.Add(pawn);
            });
            return culprits;
        }
